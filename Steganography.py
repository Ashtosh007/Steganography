...
tk.Button(frame, text="Browse", command=select_image).grid(row=0, column=2, padx=5, pady=5)
...
tk.Label(frame, text="Enter Secret Message:").grid(row=2, column=0, padx=5, pady=5)
...
tk.Button(frame, text="Encrypt & Save", command=encrypt_message).grid(row=4, column=1, pady=10)

label_image_preview = tk.Label(root)
label_image_preview.pack()

label_encrypted_preview = tk.Label(root)
...
tk.Button(root, text="Decrypt Image", command=request_password).pack(pady=10)
